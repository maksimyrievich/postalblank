...
class TCPDF {
	 ...
	 * @param $w (float) Ширина клетки. Если 0, они тянутся к правому краю страницы.
	 * @param $h (float) Минимальная высота ячейки. Клетки, продлевается автоматически, если это необходимо.
	 * @param $txt (string) Отображаемая строка.
	 * @param $border (mixed) Indicates if borders must be drawn around the cell. The value can be a number:<ul><li>0: no border (default)</li><li>1: frame</li></ul> or a string containing some or all of the following characters (in any order):<ul><li>L: left</li><li>T: top</li><li>R: right</li><li>B: bottom</li></ul> or an array of line styles for each border group - for example: array('LTRB' => array('width' => 2, 'cap' => 'butt', 'join' => 'miter', 'dash' => 0, 'color' => array(0, 0, 0)))
	 * @param $align (string) Выравнивание текста в центре или с краев. Возможные значения:<ul><li>L или пустая строка: левостороннее выравнивание</li><li>C: по центру</li><li>R: правостороннее выравнивание</li><li>J: justification (default value when $ishtml=false)</li></ul>
	 * @param $fill (boolean) Indicates if the cell background must be painted (true) or transparent (false).
	 * @param $ln (int) Указывает, где текущая позиция должна идти после вызова. Possible values are:<ul><li>0: to the right</li><li>1: to the beginning of the next line [DEFAULT]</li><li>2: below</li></ul>
	 * @param $x (float) x задание позиции в единицах пользователя
	 * @param $y (float) y задание позиции в единицах пользователя
	 * @param $reseth (boolean) if true reset the last cell height (default true).
	 * @param $stretch (int) режим растягивания текста: <ul><li>0 = disabled</li><li>1 = horizontal scaling only if text is larger than cell width</li><li>2 = forced horizontal scaling to fit cell width</li><li>3 = character spacing only if text is larger than cell width</li><li>4 = forced character spacing to fit cell width</li></ul> General font stretching and scaling values will be preserved when possible.
	 ...
	 * @param $valign (string) Вертикальное выравнивание текста (requires $maxh = $h > 0). Possible values are:<ul><li>T: TOP</li><li>M: middle</li><li>B: bottom</li></ul>. This feature works only when $ishtml=false and the cell must fit in a single page.
	...
	/**
	 * Отправить документ к заданному пункту назначения: строкой, локальным файлом или в браузер.
